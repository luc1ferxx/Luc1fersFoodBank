...
- Backend uses Java 17, Gradle wrapper 8.14.3, PostgreSQL, Redis, Kafka, Spring Session, Actuator, and Testcontainers.
...
`INIT_DB=always` runs `database-init.sql`, which drops and recreates tables. Use it only for disposable local/demo databases. Leave `INIT_DB` unset, or set `INIT_DB=never`, when data must be preserved.
...
- Keep schema changes in `backend/src/main/resources/database-init.sql` aligned with the JDBC repositories and tests. There is no Flyway or Liquibase migration framework in this repo yet.
